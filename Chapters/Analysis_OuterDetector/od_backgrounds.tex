...
\par
In \autoref{fig:od_random_trigger} a comparison between the observed events in the OD during the entirety of SR1 and those expected are shown from the Random Trigger (described in \autoref{sec:lz_detector}).
The expected rates are those described in \autoref{sec:simulated_od_requirements} and were simulated using the full model chain with the resultant pulse areas scaled by factor 4.77 determined in the previous section.
...
More worryingly there is a peak in the data at 100 phd which does is not in either of the expected distributions.
\par
In the remainder of this section, an effort is made to understand what is observed and why it is different to what was expected.
...
The OD was filled in June 2021, but between then and around the start of the first science run (SR1) in December 2021 the electronics of the detector electronic parameters were being altered during a commissioning phase.
As such, an OD background rate measurement is most reliable during SR1.
...
The noise-cut was applied as a base-cut, so the 100 keV is made up of the the noise-cut plus a pulse area (measured in photons detected or phd) cut, and similarly for 200 keV.
...
\par
There are minor fluctuations in the noise-cut rate, but these are linked to a xenon chiller and consistent with a grounding failure, thus the behaviour is not mirrored in the 100 keV or 200 keV rates.
...
Therefore what was shown in \autoref{fig:od_random_trigger} is the considered representative of the backgrounds in the OD.
...
In spherical coordinates, $\phi$ can be calculated in a similar fashion:
\begin{equation}
    \phi = \text{tan}^{-1}\bigg( \frac{\sum{\text{Ch}_\text{phd} \cdot \text{cos}_\text{phd}}}{\sum{\text{Ch}_\text{phd} \cdot \text{cos}_\text{phd}}} \bigg)
...
Regardless, this approach does provide an insight in a way not thought possible based upon optical simulations\footnote{due to the low light collection efficiency in the simulations}.
