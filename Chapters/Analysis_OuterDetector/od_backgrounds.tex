...
More worryingly there is a peak in the data at 100 phd which does is not in the simulated distributions.
\par
...
\par
To begin the journey of understanding what is in the data, the first check performed was to observe if the rate and distribution were stable over time.
Events from a month before the beginning of the first science run (SR1) until the end were monitored every week using the Random Trigger.
The rate of events above the noise-cut, 100 keV and 200 keV were measured and are shown in \autoref{fig:OD_SR1_Rate}.
The noise-cut was applied as a base-cut, so the 100 keV is made up of the the noise-cut plus a pulse area (phd) cut, and similarly for 200 keV.
...
\par
There are minor fluctuations in the noise-cut rate, but these are linked to a xenon chiller and is consistent with a grounding failure, thus the behaviour is not mirrored in the 100 keV or 200 keV rates.
