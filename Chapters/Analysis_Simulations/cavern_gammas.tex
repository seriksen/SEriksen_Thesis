...
\par
Although the LZ detector has been constructed underground to limit cosmic radiation, it does come with a drawback of being in a mine with a non-negligibly radioactive rock, shotcreat and gravel within the Davis Cavern.
...
To compensate, a generator for these $\gamma$s was used - originally described in \cite{rg_generator_ref} and is summarised below.
...
In the first stage, $\gamma$s from the cavern wall are simulation, and are propagate until they reach a surface closer to the detector, at which point they are saved, and the simulated stopped.
...
\par
This simulation considers additional cavern properties are added to the simulation.
...
\par
Although as mentioned in \cite{rg_generator_ref}, a generator for the Davis Cavern $\gamma$'s existed, it had a number of flaws.
...
Additionally, the generator was created using a GEANT4.9.5 while the rest of the simulation chain had moved on to GEANT4.10.3.p02. 
...
Although it can be seen that the previous generators had a maximum $\gamma$ energy of 10MeV, it also highlights a significant change between GEANT4.9.5 and GEANT4.10.3.p02, namely an increased rate above 9MeV.
...
As these higher energy $\gamma$'s are able to travel further into the detector they are more likely to reach the TPC - mimicking dark matter.
...
Given this, it has been suggested that the rate of $\gamma$'s above 2MeV from ($\alpha$,$\gamma$) reactions being 5 orders of magnitude higher than expected in GEANT4 simulations \cite{soudanmine_counter_point_ref, alpha_gamma_reactions_ref}.
...
\par
The energy deposit distribution from the updated generator is shown in \autoref{fig:cavern_gamma_position_distribution}.
The most significant contribution to rate is in the bottom of the Outer Detector side tanks, and events which enter the TPC enter from the sides as well as the Top and Bottom.
