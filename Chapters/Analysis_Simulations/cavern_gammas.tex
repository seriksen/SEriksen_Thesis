...
These $\gamma$'s are predominately from $O + \alpha \to Ne + \gamma$.
These are of particular concern as they have the potential to produce large signals in the OD which would trigger a veto.
\par
The livetime (l) for a simulation can be determined then by $l = \frac{\text{n.} \gamma_{\text{simulated}}}{\text{n.} \gamma_{\text{generator}}} * \text{livetime}_{\text{generator}}$
...
\par
Additionally it was noticed during the comparison that in the previous generator the distribution was not a cylinder but rather has mistakenly been created as a table.
