...
\par 
We can relate the differential cross section with respect to the momentum transfer to the scattering amplitude, $\mathcal{M}$, using Fermi's Golden Rule \cite{shaunalsum_thesis_ref}:
\begin{equation}
...
\begin{equation}
    \frac{dR}{dE_R} = \frac{A^2 \sigma_0 \rho_\chi}{2 m_\chi \mu^2} F^2(q) \int^{\infty}_{v_{min}} \frac{f(\vec{v})}{v} d\vec{v}
