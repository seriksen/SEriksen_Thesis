...
\par 
We can relate the WIMP-nucleon differential cross-section with respect to the momentum transfer to the WIMP-nucleon scattering amplitude, $\mathcal{M}$, using Fermi's Golden Rule \cite{shaunalsum_thesis_ref}:
\begin{equation}
...
\begin{equation}
    \frac{dR}{dE_R} = \frac{2\rho_{\chi}}{\pi m_{\chi}} \int^{\infty}_{v_{min}} \frac{f(\vec{v})}{v} | \mathcal{M} |^2 d\vec{v}
...
$C$ is a dimensionless parameter containing the particle physics of the interaction.
This allows for the differential WIMP-nucleon cross-section to be written as:
\begin{equation}
    \frac{d\sigma}{d|q|^2} = \frac{G_F^2}{v^2} F^2(q)
...
The zero-momentum transfer cross-section is defined as \cite{supersymetry_wimpy_again_ref}:
\cite{supersymetry_wimpy_again_ref}:
\begin{equation}
    \sigma_0 = \int^{q_{\text{max}}^2}_{0} \frac{d\sigma}{d|q|^2}(q=0) d|q|^2 = 4G^2_F \mu^2 C
\end{equation}
where the limit is just $q_\text{max}=2\mu v$.
\par
...
\end{equation}
\par
The scattering cross-section is actually comprised of a number of interactions \cite{wimp_lagrangian_ref}.
However, only two are preserved in the zero-momentum transfer limit: a spin-independent (SI) and a spin-dependent (SD) component.
...
\par
If we assume that the coupling for WIMPs to protons and neutrons is similar\footnote{so isospin is conserved} ($f_p \backsim f_n$) then the differential cross-section becomes:
\begin{equation}
    \frac{d\sigma}{d|q|^2} = \frac{\sigma_0 A^2}{4 \mu^2 v^2} F^2(q)
\end{equation}
By substituting this into \autoref{eq:wimp_differential_rate_scattering_amplitude} we arrive at the SI differential recoil spectrum for WIMP-nucleon elastic scattering:
\begin{equation}
    \frac{dR}{dE_R} = \frac{\rho_\chi}{2 m_\chi \mu^2} F^2(q) \int^{\infty}_{v_{min}} \frac{f(\vec{v})}{v} A^2 \sigma_0 d\vec{v}
