\section{Detector Overview} \label{sec:OD_detector_overview}
...
The components of this cocktail are detailed in \autoref{tab:GdLS_Components}.
...
For an 420nm incident photon the PMT quantum efficiency is in excess of 25\%, with the full wavelength shown in \autoref{fig:od_pmt_qe}.
...
The components of this cocktail are summarised in \autoref{tab:GdLS_Components}.
...
\par
The complete chemical components in the GdLS is shown in \autoref{tab:GdLS_Components}.
...
\par
\autoref{tab:Gadolinium_abundances_and_crosssections} contains the natural abundances of gadolinium.
...
\par
\autoref{fig:Gd_capture_resulting_particle_count} shows the number of discrete packets of energy over which the energy is released after the capture.
...
Where the energy goes within the atom is shown in \autoref{tab:Gd_internal_energy_conversion}.
...
\par
A less useful Figure is \autoref{fig:Gd_capture_energy_conversion_shells} shows where the energy goes in terms of electron shells. 
...
\par
The proportion of the energy transferred directly to $\gamma$'s vs via internal conversion is shown in \autoref{fig:Gd_deexcitation_energy_fractions}.
...
\par
Given what has been determined above, it is worth looking at the $\gamma$ spectrum, shown in \autoref{fig:Gd_capture_resulting_gamma_spectrum}.
...
\par
The particles released as a result of the neutron capture ($\gamma$'s and $e^{-}$'s) then follow Birk's law, shown in \autoref{eq:birkslaw}.
